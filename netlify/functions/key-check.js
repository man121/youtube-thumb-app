...
function getJSON({ hostname, path, headers = {}, timeoutMs = 5000 }) {
  return new Promise((resolve, reject) => {
    const req = https.request(
      { hostname, path, method: "GET", headers, agent },
      (res) => {
        let data = "";
  ...
  if (!hasKey) {
    return {
      ...
      body: JSON.stringify({
        ok: false,
        hasKey: false,
        reason: "Missing OPENAI_API_KEY env var on Netlify.",
      }),
    };
  }

  // Try a cheap/auth-only call: GET /v1/models
  try {
    ...
    return {
      ...
      body: JSON.stringify({
        ...
        // Minimal, safe hints (no full key exposure):
    ...
    return {
      ...
      body: JSON.stringify({
        ...
        httpStatus: 0,
        error: String(e),
        note: "Network error reaching OpenAI from Netlify.",
      }),
    };
  }
};
