...
// Visible proof JS loaded:
(function(){ const el=document.getElementById('log'); if(el){ el.textContent += 'app.js loaded ✅\n'; } console.log('app.js loaded'); })();
...
// Status tester for GET
on('btnTestAI','click', async ()=>{
  try{
    const res = await fetch(FN_PATH, { method:'GET', cache:'no-store' });
    const text = await res.text();
    log('GET ' + FN_PATH + ' → ' + res.status + '\n' + text.slice(0,200));
...
draw(); runTests(); log('INIT OK: external JS executed');
